...
# Enable Rules
soft_faile["snyk_version_is_present"]
enable_rule["snyk_version_is_present"]{ not input.setup }

enable_hard["ggshield_orb_is_present"]{ not input.setup }
enable_hard["scan_image_job_uses_snyk"]{ not input.setup }
enable_hard["workflow_includes_gitguardian"]{ not input.setup }
enable_hard["workflow_includes_scan_image"]{ not input.setup }